...
const { isMongoId } = require('./customUtils')
const { uniq, isDate, isRegExp, compareObjectId, isPrimitiveType } = require('./utils.js')
...
const compareThings = (a, b, _compareStrings) => {
  ...
  const objectIdCompare = compareObjectId(a, b)
  if (objectIdCompare !== undefined) return objectIdCompare
...
const createModifierFunction = modifier => (obj, field, value) => {
  const fieldParts = typeof field === 'string' ? field.split('.').filter(e => e !== '$') : field
  ...
  function helper (el) {
    if (fieldParts.length === 1) {
      ...
      modifierFunctions[modifier](el[fieldParts[0]], fieldParts.slice(1), value)
    }
  }
...
const getObjDotValue = (obj, field) => {
  ...
  if (fieldParts.length === 1) return R.pick([fieldParts[0]])(obj)

  if (Array.isArray(obj[fieldParts[0]])) {
    // If the next field is an integer, return only this item of the array
    const i = parseInt(fieldParts[1], 10)
    if (typeof i === 'number' && !isNaN(i)) return getObjDotValue(obj[fieldParts[0]][i], fieldParts.slice(2))

    // Return the array of values
    return {
      [fieldParts[0]]: obj[fieldParts[0]].map(el => {
  ...
  return {
    [fieldParts[0]]: getObjDotValue(obj[fieldParts[0]], fieldParts.slice(1))
  }
}
...
const areThingsEqual = (a, b) => {
  // Custom type (ex: bson.ObjectID)
  const objectIdCompare = compareObjectId(a, b)
  if (objectIdCompare !== undefined) return objectIdCompare === 0
