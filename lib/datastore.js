...
class Datastore extends EventEmitter {
   ...
   * @param {Boolean} options.timestampData Optional, defaults to false. If set to true, createdAt and updatedAt will be created and populated automatically
   *   (if not specified by user)
   * @param {Boolean} options.inMemoryOnly Optional, defaults to false
   * @param {String} options.nodeWebkitAppName Optional, specify the name of your NW app if you want options.filename to be relative to the directory where
   *                                            Node Webkit stores application data such as cookies and local storage (the best place to store data in my
   *   opinion)
   * @param {Boolean} options.autoload Optional, defaults to false
   * @param {Function} options.onload Optional, if autoload is used this will be called after the load database with the error object as parameter. If you
   *   don't pass it the error will be thrown
  ...
  ensureIndex () {
    if (customUtils.isMongoose()) {
      const [indexFields = {}, indexOptions, callback = () => {}] = arguments
      if (!indexFields) {
        const err = new Error('Cannot create an index without a fieldName')
        err.missingFieldName = true
        return callback(err)
      }
      let fieldName = Object.keys(indexFields)
      fieldName = fieldName.length === 1 ? fieldName[0] : fieldName
      if (this.indexes[fieldName]) return callback(null)

      this.indexes[fieldName] = new Index({ fieldName, ...indexOptions })
      // With this implementation index creation is not necessary to ensure TTL but we stick with MongoDB's API here
      // if (options.expireAfterSeconds !== undefined) this.ttlIndexes[fieldName] = options.expireAfterSeconds
      ...
      if (options.expireAfterSeconds !== undefined) this.ttlIndexes[options.fieldName] = options.expireAfterSeconds // With this implementation index creation
      // is not necessary to ensure TTL but we
      // stick with MongoDB's API here
   ...
   *
   * @param {Query} query
   * @param {Boolean} dontExpireStaleDocs Optional, defaults to false, if true don't remove stale docs. Useful for the remove function which shouldn't be
   *   impacted by expirations
      ...
      cb => {
        ...
        Object.keys(query).forEach(k => {
          if (typeof query[k] === 'string' || typeof query[k] === 'number' || typeof query[k] === 'boolean' ||
            isDate(query[k]) || query[k] === null || customUtils.isMongoId(query[k])) {
            usableQueryKeys.push(k)
          }
        })
        // For a basic match
        // if is sparse index and the value is null, cannot use the index.如果是稀疏索引，不走索引，全表扫描，不然使用null查稀疏索引永远会返回空数组。
        basicQueryKeys = R.reject(key => (query[key] === null && this.indexes[key].sparse), R.intersection(usableQueryKeys, indexNames))
        if (basicQueryKeys.length > 0) {
          return cb(null, this.indexes[basicQueryKeys[0]].getMatching(query[basicQueryKeys[0]]))
        }
        ...
        indexNames.forEach(indexName => {
          ...
          if (R.intersection(subIndexNames, usableQueryKeys).length === subIndexNames.length) {
            compoundQueryKeys.push(subIndexNames)
          }
        })

        if (compoundQueryKeys.length > 0) {
          return cb(null, this.indexes[compoundQueryKeys[0]].getMatching(R.pick(compoundQueryKeys[0])(query)))
        }
  ...
  insertMany (docs, safe, cb) {
    return this.insert(docs, safe, cb)
  }

  /**
   * Count all documents matching the query
   * @param {Object} query MongoDB-style query
   * @param {Object} option MongoDB-style option,add for mongoose
   * @param {Function} callback Optional callback, signature: err, count
   */
  count (query, option, callback) {
    if (option && option instanceof Function && !callback) callback = option
  ...
  /**
   * Distinct all documents matching the query(distinct by ES6 Set)
   * If no callback is passed, we return the cursor so that user can limit, skip and finally exec
   * @param {Object} key distinct field
   * @param {Query} query MongoDB-style query
   * @param {Object} unused for mongoose
   * @param {Function} callback Optional callback, signature: err, docs
   */
  distinct (key, query = {}, unused, callback) {
    const cursor = new Cursor(this, query, function (err, docs, callback) {
      if (err) { return callback(err) }
      const res = [...new Set(docs.map(doc => model.getDotValue(doc, key)).filter(v => v !== undefined))]
   ...
   *                 options.returnUpdatedDocs Defaults to false, if true return as third argument the array of updated matched documents (even if no change
   *   actually took place)
  ...
  _update (query, updateQuery, options = {}, cb) {
    ...
    const returnUpdatedDocs = options.returnUpdatedDocs !== undefined ? options.returnUpdatedDocs : false
    const returnNew = options.new !== undefined ? options.new : returnUpdatedDocs
    ...
    async.waterfall([
        ...
        cursor.limit(1)._exec((err, docs) => {
          ...
          if (docs.length === 1) return cb()
            ...
            try {
              ...
              try {
                const mergedUpsertQuery = Object.assign({}, query, updateQuery.$setOnInsert || {})
                if (updateQuery.$setOnInsert) {
                  delete updateQuery.$setOnInsert
                }
                toBeInserted = model.modify(model.deepCopy(mergedUpsertQuery, true), updateQuery)
              } catch (err) {
                return callback(err)
              }
            }
      ...
      () => { // Perform the update
        ...
        this.getCandidates(query, (err, candidates) => {
          ...
          try {
            for (const candidate of candidates) {
              if (model.match(candidate, query) && (multi || numReplaced === 0)) {
                ...
                modifiedDoc = model.modify(candidate, updateQuery, query)
          ...
          // Update the datafile
          const beforeUpdatedDocs = modifications.map(x => x.oldDoc)
          const updatedDocs = modifications.map(x => x.newDoc)
          this.persistence.persistNewState(updatedDocs, err => {
            if (err) return callback(err)
            if (!options.returnUpdatedDocs) {
              return customUtils.isMongoose() ? callback(null, { result: { ok: numReplaced } }) : callback(null, numReplaced)
            } else {
              let updatedDocsDC = [];
              (returnNew ? updatedDocs : beforeUpdatedDocs).forEach(doc => {
                updatedDocsDC.push(model.deepCopy(doc))
              })
              if (!multi) updatedDocsDC = updatedDocsDC[0] || null
              return customUtils.isMongoose() ? callback(null, { [multi ? 'result' : 'value']: updatedDocsDC }) : callback(null, numReplaced, updatedDocsDC)
            }
          })
        })
      }])
  ...
  updateOne (query, updateQuery, options = {}, cb) {
    return this.update(query, updateQuery, { ...options, multi: false }, cb)
  }

  updateMany (query, updateQuery, options = {}, cb) {
    return this.update(query, updateQuery, { ...options, multi: true }, cb)
  }

  findAndModify (query, sort, updateQuery, options = {}, cb) {
    return this.update(query, updateQuery, { ...options, returnUpdatedDocs: true, sort }, cb)
  }
  ...
  _remove (query, options, cb) {
    ...
    this.getCandidates(query, true, (err, candidates) => {
      ...
      this.persistence.persistNewState(removedDocs, err => {
        if (err) return callback(err)
        return callback(null, customUtils.isMongoose() ? { ok: numRemoved } : numRemoved)
      })
    })
  }
  ...
  deleteOne (query, options, cb) {
    return this.remove(query, { ...options, multi: false }, cb)
  }

  deleteMany (query, options, cb) {
    return this.remove(query, { ...options, multi: true }, cb)
  }
}
